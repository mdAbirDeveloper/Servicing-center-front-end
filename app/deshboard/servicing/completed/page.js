    ...
    const fetchCompleted = async () => {
      try {
        const res = await fetch(
          "https://servicing-center-server.vercel.app/api/v1/repair/completed",
          {
            headers: {
        ...
        if (res.ok) {
          ...
          const years = [
            ...new Set(
              fetchedData.map((item) => new Date(item.updatedAt).getFullYear())
            ),
          ];
  ...
  useEffect(() => {
    ...
    if (selectedYear) {
      filtered = filtered.filter(
        (item) =>
          new Date(item.updatedAt).getFullYear() === parseInt(selectedYear)
      );
    }
    ...
    if (selectedMonth) {
      ...
      filtered = filtered.filter(
        (item) =>
          new Date(item.updatedAt).getMonth() === parseInt(selectedMonth)
      );
    }
